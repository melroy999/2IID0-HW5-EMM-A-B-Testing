...
repetitions = 500;
euclidean_distance_results = zeros(repetitions, 9);
results_retained = zeros(repetitions, 9);
...
for i = 1:repetitions
    ...
    V_experiment_attribute_mean = zeros(7, 1);
    V_experiment_attribute_std = zeros(7, 1);
    V_experiment_attribute_mode = zeros(7, 1);
    for j = 1:7
    ...
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(ignore_missing_values_V, beta_original);
    
    column = column + 1;
    attribute_average_substitution_V = attribute_average_substitution(V_experiment, V_experiment_attribute_mean);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(attribute_average_substitution_V, beta_original);
    
    column = column + 1;
    record_average_substitution_V = record_average_substitution(V_experiment);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(record_average_substitution_V, beta_original);
    
    column = column + 1;
    attribute_majority_vote_V = attribute_majority_vote(V_experiment, V_experiment_attribute_mode);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(attribute_majority_vote_V, beta_original);
    
    column = column + 1;
    record_majority_vote_V = record_majority_vote(V_experiment);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(record_majority_vote_V, beta_original);
    
    column = column + 1;
    record_minimum_substitution_V = record_minimum_substitution(V_experiment);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(record_minimum_substitution_V, beta_original);
    
    column = column + 1;
    record_maximum_substitution_V = record_maximum_substitution(V_experiment);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(record_maximum_substitution_V, beta_original);
    
    column = column + 1;
    rating_target_averaging_V = rating_target_averaging(V_experiment, 0);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(rating_target_averaging_V, beta_original);
    
    column = column + 1;
    rating_target_averaging_correction_V = rating_target_averaging(V_experiment, 0.425);
    [euclidean_distance_results(i,column), results_retained(i,column)] = get_evaluations(rating_target_averaging_correction_V, beta_original);
end
